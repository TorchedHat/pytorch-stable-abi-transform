...
def parse_ast(header: Path, include_dir: Path) -> dict:
    ...
    if result.returncode != 0:
        print(
            f"warning: clang parse errors in {header.name} (extracting what we can)",
            file=sys.stderr,
        )
...
def extract_functions(ast: dict) -> list[StableOp]:
    ...
    def visitor(node, kind, name, namespace):
        ...
        first_is_tensor = bool(params and "Tensor" in params[0].get("type", {}).get("qualType", ""))
        ops.append(
            StableOp(
                name=name,
...
def extract_macros(stable_dir: Path) -> list[StableMacro]:
    ...
    if macros_h.exists():
        ...
        if "STD_CUDA_KERNEL_LAUNCH_CHECK" in text:
            macros.append(
                StableMacro("STD_CUDA_KERNEL_LAUNCH_CHECK", "C10_CUDA_KERNEL_LAUNCH_CHECK")
            )
...
def generate_rules_h(
    ...
    include_rules = [
        ...
        (
            ...
            ["torch/csrc/stable/accelerator.h", "cuda_runtime.h", "", "", ""],
            False,
        ),
        (
        ...
        (
            "ATen/Dispatch.h",
            ["torch/headeronly/core/Dispatch.h", "torch/headeronly/core/Dispatch_v2.h", "", "", ""],
            False,
        ),
        (
            "ATen/ATen.h",
...
def main():
    ...
    method_op_names = {
        op.name
        for op in ops
        if op.is_method_like and op.name not in tensor_methods and not op.name.endswith("_out")
    }
    overlap = method_op_names & set(stable_methods)
    if overlap:
        print(f"error: methods in both rewrite rules and whitelist: {overlap}", file=sys.stderr)
        sys.exit(1)
